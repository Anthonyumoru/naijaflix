 ...
 *  - Encrypted video caching using AES-GCM (Web Crypto API)
 *  - Videos stored in IndexedDB as encrypted chunks — NOT in Downloads folder
 *  - Decrypted on-the-fly only during playback within your site
 *  - Range request support for video seeking
 *  - Message-based API for download / list / delete / check
 */
...
const CHUNK_SIZE = 5 * 1024 * 1024; // 5 MB per encrypted chunk

const VIDEO_HOSTS = [
  "moviebox-stream.umoruanthony345.workers.dev",
];
...
async function getMasterKey() {
  ...
  const key = await crypto.subtle.generateKey(
    ...
    false, // non-extractable — cannot be exported or copied out
    ["encrypt", "decrypt"]
  );
...
// ─── Serve Cached Encrypted Video (with Range support) ───────────────────────
...
self.addEventListener("fetch", (e) => {
  ...
  // Video requests: try encrypted cache first, fall back to network
  ...
  const sendProgress = (progress) => {
    if (clientId) {
      self.clients.get(clientId).then((client) => {
        if (client) {
          client.postMessage({ type: "DOWNLOAD_PROGRESS", url, progress });
        }
      });
    }
  };
        ...
        (async () => {
          try {
            ...
            if (clientId) {
              ...
              client?.postMessage({ type: "DOWNLOAD_COMPLETE", ...result });
            }
          } catch (err) {
            if (clientId) {
              const client = await self.clients.get(clientId);
              client?.postMessage({ type: "DOWNLOAD_ERROR", url, error: err.message });
            }
          }
        })()
      );
